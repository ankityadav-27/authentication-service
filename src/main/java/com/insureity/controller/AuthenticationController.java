...
public class AuthenticationController {
	...
	/**
	 * Login using agent details
	 * 
	 * @param userlogincredentials
	 * @return
	 */
	@PostMapping(value = "/login")
	public ResponseEntity<?> login(@RequestBody Agent userlogincredentials) {
		...
		if (userdetails.getPassword().equals(userlogincredentials.getUpassword())) {
			...
			return new ResponseEntity<>("Invalid Login Credentials!!!", HttpStatus.FORBIDDEN);//use enum, remove hard coding
		}
	}
